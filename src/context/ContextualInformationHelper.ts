...
import {CollectionViewDOMConfig, EXTRA_ACTION_ATTRIBUTE_NAME} from "../ConfigurationTypes";
...
export class ContextualInformationHelper {
    public static IDENTIFIER: string = 'context-id';
    ...
    insertDisplayElementForCollectionItem(view: CollectionView, renderer: CollectionViewRenderer, containerEl: HTMLElement, collectionName: string, item: any, prepend: boolean = false): HTMLElement {
        ...
        const uiConfig: CollectionViewDOMConfig = view.getCollectionUIConfig();

        const querySelector = `${uiConfig.resultsElement.type}[${ContextualInformationHelper.IDENTIFIER}="${resultDataKeyId}"]`;
        let childEl = <HTMLElement>containerEl.querySelector(querySelector);
        if (!childEl) {
            let shouldDisplayTheNewItem: boolean = true;
