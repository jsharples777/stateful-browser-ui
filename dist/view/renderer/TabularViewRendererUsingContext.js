...
export class TabularViewRendererUsingContext {
    ...
    insertDisplayElementForCollectionItem(containerEl, collectionName, item) {
        if (this.tableBodyEl)
            ContextualInformationHelper.getInstance().insertDisplayElementForCollectionItem(this.view, this, this.tableBodyEl, collectionName, item, true);
    }
    removeDisplayElementForCollectionItem(containerEl, collectionName, item) {
        if (this.tableBodyEl)
            ContextualInformationHelper.getInstance().removeDisplayElementForCollectionItem(this.view, this, this.tableBodyEl, collectionName, item);
    }
    updateDisplayElementForCollectionItem(containerEl, collectionName, item) {
        if (this.tableBodyEl)
            ContextualInformationHelper.getInstance().updateDisplayElementForCollectionItem(this.view, this, this.tableBodyEl, collectionName, item);
    }
    ...
    setDisplayElementsForCollectionInContainerForNextPage(containerEl, collectionName, newState) {
        ...
        if (this.currentPage === 1) {
            if (this.tableBodyEl)
                browserUtil.removeAllChildren(this.tableBodyEl);
        }
        ...
        for (let index = startItemIndex; index < endItemIndex; index++) {
            ...
            if (this.tableBodyEl)
                lastChildEl = ContextualInformationHelper.getInstance().insertDisplayElementForCollectionItem(this.view, this, this.tableBodyEl, collectionName, item);
        }
...
TabularViewRendererUsingContext.SORT_DIRECTION_ATTRIBUTE = 'sort-direction';
TabularViewRendererUsingContext.SORTABLE_TH_CLASS = 'sortable-th';
TabularViewRendererUsingContext.NON_SORTABLE_TH_CLASS = 'non-sortable-th';
